...
class SkeletonManager final : SingletonInterface<SkeletonManager> {
__SINGLETON_INTERFACE(SkeletonManager)
...
public:
	...
	static std::shared_ptr<const SkeletonResource> GetSkeleton(const std::string& name);
	...
	static void Transfer(const std::string& name, std::shared_ptr<SkeletonResource>& data);
...
private:
	std::unordered_map<std::string, std::shared_ptr<SkeletonResource>> instanceList;
};
