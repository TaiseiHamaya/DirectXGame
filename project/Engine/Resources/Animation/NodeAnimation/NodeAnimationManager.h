...
class NodeAnimationManager final : SingletonInterface<NodeAnimationManager> {
	__SINGLETON_INTERFACE(NodeAnimationManager)
...
public:
	...
	static std::shared_ptr<const NodeAnimationResource> GetAnimation(const std::string& name);
	...
	static void Transfer(const std::string& name, std::shared_ptr<NodeAnimationResource>& data);
...
private:
	std::unordered_map<std::string, std::shared_ptr<NodeAnimationResource>> instanceList;
};
