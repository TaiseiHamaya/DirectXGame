...
void Texture::load_texture(const std::string& FilePath, uint32_t heapIndex) {
	...
	// textureResourceに転送
	DirectXDevice::GetDevice()->CreateShaderResourceView(textureResource, &srvDesc, textureSrvHandleCPU);
}
...
void Texture::create_texture_resource(const DirectX::TexMetadata& metadata) {
	...
	hr = DirectXDevice::GetDevice()->CreateCommittedResource(&heapPropertis, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&textureResource));
	assert(SUCCEEDED(hr));
}
...
ID3D12Resource* Texture::upload_texture_data(const DirectX::ScratchImage& mipImages) {
	...
	DirectXCommand::SetBarrier(textureResource, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_GENERIC_READ);
