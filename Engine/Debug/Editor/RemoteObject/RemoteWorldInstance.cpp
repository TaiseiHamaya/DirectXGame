...
void RemoteWorldInstance::initialize(Reference<WorldInstance> self_) {
	self = self_;
}
...
void RemoteWorldInstance::draw_inspector() {
	...
	if (self) {
		ImGui::Checkbox("Active", &self->isActive);
		self->transform.debug_gui();
	}
	else {
		ImGui::Text("Error");
		ImGui::Text("Not fount instance.");
	}
}
...
void RemoteWorldInstance::draw_hierarchy(Reference<IRemoteObject>& select) {
	...
	if (self) {
		// 子がいる場合
		if (!(childrenFolders.empty() && childrenInstances.empty())) {
			int flags =
	...
	// なんかおかしい
		...
		if (ImGui::Selectable(std::format("Error : NotFound {}##{}", hierarchyName, (void*)this).c_str(), isSelected)) {
			select = this;
		}
	}
}
